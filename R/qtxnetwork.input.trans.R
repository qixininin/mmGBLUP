...
qtxnetwork.input.trans <- function(geno_data, pheno_data, geno_output_prefix, pheno_output_prefix)
{
  ...
  for(c in 1:traitNum)
  {
    ...
    write.table(pheno_data[,c(1,c+1)], file = printer, append = T, na = ".",
                row.names = F, col.names = T, quote = F,sep = "\t", eol = ";\n")
